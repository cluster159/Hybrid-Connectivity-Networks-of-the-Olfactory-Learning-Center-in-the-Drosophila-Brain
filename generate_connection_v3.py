    ...
    def partial_shuffle_connections(self, KC_class, network_number=10, shuffle_ratio=1.0, seed=100, binary=True):
        ...
        for network_i in range(network_number):
            ...
            if binary:
                ...
                # sn.heatmap([summation_pre,new_pre])
                # plt.show()
                ...
                # sn.heatmap([summation_post,new_post])
                # plt.show()

                if np.count_nonzero(summation_post-new_post) > 0:
                    raise BaseException("The connection matrix is wrong!!")
            # else:
            #     summation_pre = np.sum(weight_matrix,axis=0)
            #     new_pre = np.sum(tmp_connection_matrix,axis=0)
            #     if np.count_nonzero(summation_pre-new_pre) > 0:
            #         raise BaseException("The connection matrix is wrong!!")

            # sn.heatmap(data=original_connection_matrix)
            # plt.show()
            # sn.heatmap(data=tmp_connection_matrix)
            # plt.show()
            # differ = np.count_nonzero(original_connection_matrix-tmp_connection_matrix)/np.count_nonzero(original_connection_matrix)
            # print(np.count_nonzero(original_connection_matrix),np.count_nonzero(tmp_connection_matrix),differ)
            ...
            # correlation_matrix = cosine_similarity(tmp_connection_matrix)
    ...
    def construct_random_claw_model(self, shuffled_times=100, completeness=1.0, dataset='FlyEM', sampled_ratio_list = [1.0,1.0,1.0], 
        ...
        for shuffled_index in range(shuffled_times):
            ...
            if shuffled_table is None:
                ...
                connectivity_collection.append(connectivity.transpose().ravel())
        # G_correlation_collection = np.array(G_correlation_collection)
        # z_score_G_correlation = (original_G_correlation.ravel()-np.average(G_correlation_collection,axis=0))/np.std(G_correlation_collection,axis=0)
        # sn.heatmap(data=np.reshape(np.average(G_correlation_collection,axis=0),(len(self.G_list),len(self.G_list))))
        # plt.savefig("G_average.png")
        # plt.close()
        # sn.heatmap(data=np.reshape(np.std(G_correlation_collection,axis=0),(len(self.G_list),len(self.G_list))))
        # plt.savefig("G_STD.png")
        # plt.close()
        
        # z_score_G_correlation = np.reshape(z_score_G_correlation,(len(self.G_list),len(self.G_list)))

        # print(z_score_G_correlation.shape)
        # g = sn.clustermap(data=z_score_G_correlation,xticklabel=self.G_list)
        ...
        # plt.savefig(f'Random_claw_model_z_score_{completeness}_G_correlation.png',dpi=500)
        # plt.close()
...
def plot_Lshape():
    ...
    # ax.set_xticklabels([f"{G}" for G in network.G_list], rotation=90)
    ...
    # ]
    # col_sep_indices = [
    #     len(network.Cluster_to_Glomerulus[1]),
    #     len(network.Cluster_to_Glomerulus[1]) + len(network.Cluster_to_Glomerulus[2])
    # ]
    ...
    # # Add vertical separation lines using ax.axvline
    # for col in col_sep_indices:
    #     ax.axvline(col, color='red', linewidth=2)
...
if __name__=='__main__':
    ...
    input()
    # plot_Lshape()
    plot_connection_matrix_according_to_function()
    input()
    file_name = 'ConnectionSetting.pickle'
    path = 'tmp_files/'
    Load = True
    ...
    # c.check_claw_num_difference_with_connection_table()
    # c.check_bouton_num_difference_with_connection_table()
    # c.match_claw_bouton_reconstruct_connection_table()
    # c.read_FAFB_connection_csv()
    # c.Calculate_Cluster_to_KC_specificity()
    # c.Calculate_KC_from_Cluster_specificity()
    # c.Calculate_Cluster_to_KC_specificity(dataset='FAFB')
